define(['jquery', 'knockout', 'compevent', 'bignumber','billcheck','/iwebap/erm/js/up.js', '/iwebap/js/BillFormActionUtils.js','artDialog','/iwebap/erm/js/SimpleMap.js' , 'director','sco.message','/iwebap/js/ImageBrowser.js','/iwebap/erm/js/jquery_resize.js','/iwebap/ssccloud/credit/credit.js','/iwebap/ssccloud/pub/getsysparameter.js'], function($, ko, compevent, bignumber,billcheck,up) {
	...
	ctrl.init = function(viewModel, app, billForm, appModel) {
		...
		//add by wangchao 2020-01-08 表头字段费用类别联动表体字段收支项目 =================================
		if(getRequest().tradetype !="2647" && getRequest().tradetype !="264X-Cxx-bxld"){
			if(ctrl.headTable.meta['zyx4'].default){
				var newvalue = ctrl.headTable.meta['zyx4'].default.value;
				if(newvalue!=null&&newvalue.length>0){
					var szxmcfg = {};
					var wherePart = " and (def5 = '" + newvalue + "'   or def5 = '~' )";
					if(getRequest().tradetype.indexOf("264")>=0){
						if(ctrl.viewModel.body_1arap_bxbusitem.meta['szxmid'].refparam){
							szxmRefparam = JSON.parse(ctrl.viewModel.body_1arap_bxbusitem.meta['szxmid'].refparam);
							szxmRefparam.wherePart = " and def5 = '" + newvalue + "'";
							if(ctrl.viewModel.body_1arap_bxbusitem && ctrl.viewModel.body_1arap_bxbusitem.meta['szxmid']){
								ctrl.viewModel.body_1arap_bxbusitem.setMeta("szxmid","refparam",JSON.stringify(szxmRefparam));
							}
						}
					}else if(getRequest().tradetype.indexOf("263X-Cxx-001")>=0){
						if(ctrl.viewModel.body_1jk_busitem.meta['szxmid'].refparam){
							szxmRefparam = JSON.parse(ctrl.viewModel.body_1jk_busitem.meta['szxmid'].refparam);
							szxmRefparam.wherePart = " and def5 = '" + newvalue + "'";
							if(ctrl.viewModel.body_1jk_busitem && ctrl.viewModel.body_1jk_busitem.meta['szxmid']){
								ctrl.viewModel.body_1jk_busitem.setMeta("szxmid","refparam",JSON.stringify(szxmRefparam));
							}
						}
				...
				if(checkLinkAttr(event)||checkLinkRefAttr(event)){
					if(event.dataTable == ctrl.dataTableIDs.head){
						...
						ctrl.app.serverEvent().addDataTables(ctrl.dataTableIDs).addParameter("tableID",event.dataTable).addParameter("field",event.field).addParameter("newValue",""+event.newValue).addParameter("oldValue",""+event.oldValue).fire({
							...
							async : false,
							success: function(data){
								//add by zhongzh7 粤运项目 费用分摊明细 选择承担单位后，更新线路、往来明细项所属财务组织档案 20180709 start
								if(event.field == 'assume_org'){
									
									var lineRefparam = {};
									//获取费用承担明细-承担单位pk
									lineRefparam.pk_org=event.newValue;
									//更新线路档案  参照财务组织
									ctrl.app.dataTables["body_1er_cshare_detail"].setMeta('defitem2','refparam',JSON.stringify(lineRefparam));
									//更新往来明细项 参照财务组织
									ctrl.app.dataTables["body_1er_cshare_detail"].setMeta('defitem3','refparam',JSON.stringify(lineRefparam));
								}
								//add by zhongzh7 粤运项目 费用分摊明细 选择承担单位后，更新线路、往来明细项所属财务组织档案  20180709 end
							}
						});					
					}
				}
...
//				&&  new bignumber(ctrl.busitemTables[i].getCurrentRow().data.amount.value) != 0 
	...
	function checkLinkAttr(event) {
		var headLinkAttrs = ["jobid"/*项目*/,"pk_org"/*组织*/,"djrq"/*单据日期*/,"bzbm"/*币种*/,"bbhl"/*汇率*/,"dwbm_v"/*报销人单位*/,"dwbm","deptid_v"/*报销人部门_v*/,"deptid"
							,"jkbxr"/*借款报销人*/,"skyhzh"/*个人银行账户*/,"receiver"/*收款人*/,"fydeptid_v"/*费用承担部门_v*/,"fydeptid","fydwbm_v"/*费用承担单位_v*/,"fydwbm"/*费用承担单位_v*/,"pk_payorg_v"/*支付单位_v*/,"pk_payorg","szxmid"/*收支项目*/,"paytarget"/*收款对象*/,"hbbm"/*供应商*/,"customer"/*客户*/,"custaccount"/*客商银行账户*/,"freecust"/*散户*/,"zyx25"/*是否有合同*/];
	...
	function loadData() {
		...
		ctrl.app.serverEvent().addDataTables(ctrl.dataTableIDs,"all").addParameter("uistate", appModel.getUIState()).addParameter("pk_billtemplet", pk_billtemplet).fire({
			...
			success: function(data) {
				...
				ctrl.headTable.setMeta('pk_contractno','refparam',JSON.stringify(headApctRefparam));
				//end
				//ctrl.billForm.triggerEditFormular();//触发公式联动，很影响效率
				//加载报销标准，放在公式联动后面，根据默认值以及公式计算的结果来加载报销标准
				/*ctrl.app.serverEvent().addDataTables(ctrl.dataTableIDs,"all").addParameter("uistate", appModel.getUIState()).addParameter("pk_billtemplet", pk_billtemplet).fire({
					ctrl: 'JKBXDefDataController',
					method: 'loadReim',
					async : false,
