select org_financeorg.name || '对账函' title,
       ...
       nvl(qc.qmye, 0) - nvl(znxs.ckwkpje, 0) + nvl(fl.flwhcje, 0) khyecw,
       nvl(qc.qmye, 0) - nvl(znxs.ckwkpje, 0) + nvl(fl.flwhcje, 0) -
       nvl(qc.qcye, 0) - nvl(khfk.khfkje, 0) - nvl(fl.flje, 0) +
 ...
 inner join org_dept
  ...
  left outer join (select pk_group,
                          pk_org,
                          customer,
                          pk_dept,
                          sum(qcye) as qcye,
                     ...
                     from (select gl_balance.pk_group,
                                  ...
                                  sum(case
                                        when gl_balance.adjustperiod <= '12' then
                                         nvl(gl_balance.creditamount, 0) -
                                         nvl(gl_balance.debitamount, 0)
                                        else
                                         0
                           ...
                           union all
                           select gl_detail.pk_group typecode,
                            ...
                            group by gl_detail.pk_group,
                                     ...
                                     gl_docfree1.f4,
                                     gl_docfree1.f1) temp
                    group by pk_group, pk_org, customer, pk_dept
